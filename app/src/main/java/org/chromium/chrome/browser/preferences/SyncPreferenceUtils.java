...
public class SyncPreferenceUtils {
    ...
    public static String getSyncStatusSummary(Context context) {
        ...
        if (!AndroidSyncSettings.get().isMasterSyncEnabled()) {
            return res.getString(com.us.kinscape.R.string.sync_android_master_sync_disabled);
        }

        if (profileSyncService == null) {
            return res.getString(com.us.kinscape.R.string.sync_is_disabled);
        }
        ...
        if (profileSyncService.getProtocolErrorClientAction()
            ...
            return res.getString(com.us.kinscape.R.string.sync_error_upgrade_client,
                    BuildInfo.getInstance().hostPackageLabel);
        }

        if (profileSyncService.hasUnrecoverableError()) {
            return res.getString(com.us.kinscape.R.string.sync_error_generic);
        }
        ...
        if (syncEnabled) {
            if (!profileSyncService.isSyncActive()) {
                return res.getString(com.us.kinscape.R.string.sync_setup_progress);
            }

            if (profileSyncService.isPassphraseRequiredForDecryption()) {
                return res.getString(com.us.kinscape.R.string.sync_need_passphrase);
            }
            return context.getString(
                    com.us.kinscape.R.string.account_management_sync_summary, accountName);
        }

        if (ChromeFeatureList.isEnabled(ChromeFeatureList.UNIFIED_CONSENT)) {
            return context.getString(
                    com.us.kinscape.R.string.account_management_sync_off_summary, accountName);
        }
        return context.getString(com.us.kinscape.R.string.sync_is_disabled);
    }
